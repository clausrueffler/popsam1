...
for (i in 1:sim) {# executes the function sim times
  results[, i] <- environmental.stochasticity.ext(N0, tmax, possible_outcomes, probabilities) # produces a time series and stores it in the i-th column of the matrix results
}
...
matlines(row.means, type="l", col="Black")
```
...
matlines(log(time.series.arith+1), type="l", col="4")
```
...
matlines(log(time.series.geo+1), type="l", col="Black")
```
