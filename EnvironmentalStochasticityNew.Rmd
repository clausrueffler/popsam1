...
sample(x = possible_outcomes, size = tmax, prob = probabilities, replace = TRUE) # produces a vector of tmax random numbers taking the value 0.51  1.51
```
...
We can also introduce an *extinction threshold*. If the population size drops below this threshold, then the population is extinct and cannot recover. A natural choice for such a threshold is 1.
...
plot(0:tmax, time_series, type="l", ylim=c(0,max(time_series)), xlab="time", ylab="population size")
```

Alternatively, we can also plot the time series on a log-scale by taking the natural logarithm of population size.

```{r eval=FALSE}
plot(0:tmax, log(time_series + 1), type="l", ylim=c(0,log(max(time_series + 1))), xlab="time", ylab="population size")
```

Note that for the log-scale we add 1 to all entries in the time series. The reason is that $\ln(N)$ for $N<1$ is negative.

At each instance this code is executed (starting from the line in which the vector *growth_factors* is defined), we obtain a different random time series. If you produce several time series, you will note that in some cases the population goes extinct while in others the population is still present after *tmax* time steps.
...
Next, let us add a line that shows how the population changes if each year it grows according to the arithmetic mean growth factor $\bar\lambda$.
...
```
-->
The following piece of code adds the desired line, both on the natural and log-scale

```{r eval=FALSE}
time_series_geom <- c()
time_series_geom[1] <- N0
for (t in 1:tmax) {
  time_series_geom[t+1] <- max(0,time_series_geom[t]*lambda_mean)
}

# plots on natural scale
plot(0:tmax, time_series, type="l", ylim=c(0, max(time_series)), xlab="time", ylab="population size")
lines(0:tmax, time_series_geom, type="l", col="Red")

# plots on log-scale
plot(0:tmax, log(time_series +1), type="l", ylim=c(0, max(log(time_series_geom + 1))), xlab="time", ylab="population size")
lines(0:tmax, log(time_series_geom + 1), type="l", col="Red")
```
...
plot(0:tmax, log(results[, 1] + 1), type="l", ylim=c(0, log(max(time_series_geom, results) + 1)), col=cl[1], xlab="time", ylab="ln(population size)") # plot the result from the first simulation
...
plot(0:tmax, log(results[, 1] + 1), type="l", ylim=c(1, log(max(time_series_geom, results) + 1)), col=cl[1], xlab="time", ylab="ln(population size)")
for (i in 2:sim) {
  lines(0:tmax, log(results[, i] + 1), type="l", col=cl[i])
}
lines(0:tmax, log(time_series_geom + 1), type="l", col="4")
...
# we can add mean of all time series to the previous graph
lines(0:tmax, log_row_means, type="l", col="Black")
```
...
\end{equation}
$$
Let us add a line showing $\hat N(t)$ according to this formula to our plots.
...
```{r eval=FALSE, echo=FALSE}
plot(0:tmax, log(results[, 1] + 1), type="l", ylim=c(0, log(max(time_series_geom, results) + 1)), col=cl[1], xlab="time", ylab="ln(population size)")
for (i in 2:sim) {
  lines(0:tmax, log(results[, i] + 1), type="l", col=cl[i])
}
lines(0:tmax, log(time_series_geom + 1), type="l", col="4")

# we add mean of all time series to the previous graph
lines(0:tmax, log_row_means, type="l", col="Black")
# we add the expected mean trajectory to the previous graph
lines(0:tmax, log(time_series_expected + 1), type="l", col="Black")
```
