...
sample(x = possible_outcomes, size = 10, prob = probabilities, replace = TRUE) # produces a vector of 10 random numbers taking the value 0.91  1.11
```
...
growth_factors <- sample(x = possible_outcomes, size = tmax, prob = probabilities, replace = TRUE)
```
...
Stochastic time series without extinction.
...
for (t in 1:tmax) {# for-loop that projects the population n time steps into the future
...
Assuming that *N* describes the *absolute population size* we in this code set any population size less than one equal to zero (half a fox does not exist). In other words, if $N < 1$ at any time *t*, then the we consider the population extinct from that time point onward. Next, we plot the resulting time series on a log-scale.

```{r eval=FALSE}
plot(0:tmax, time_series, type="l", ylim=c(0,max(time_series)), xlab="time", ylab="population size") # Note that we add 1 to all entries in the time series. The reason is that ln(0) is not defined.
```

```{r eval=FALSE}
plot(0:tmax, time_series+1, type="l", log="y", ylim=c(1,max(time_series)), xlab="time", ylab="population size") # Note that we add 1 to all entries in the time series. The reason is that ln(0) is not defined.
```

At each instance this code is executed (starting from the line in which the vector *growth_factors* is defined), we obtain a different random time series. If you produce several time series, you will note that in some cases the population goes extinct while in others the population is still present after *n* time steps.
