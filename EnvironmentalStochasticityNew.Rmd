...
sample(x = possible_outcomes, size = tmax, prob = probabilities, replace = TRUE) # example of stochastic sequence of lambda-values
```
...
environmental_stochasticity <- function(N0, tmax, possible_outcomes, probabilities) {
  time_series <- c() # creates an empty vector in which the time series will be stored
  time_series[1] <- N0 # enters the starting value in the first position of that vector
  for (t in 1:tmax) {# for loop that iterates the population dynamics tmax times
    lambda <- sample(possible_outcomes, 1, probabilities, replace = TRUE) # new lambda value is chosen randomly
...
Alternatively, we can plot the time series on a log-scale by taking the natural logarithm of population size.
...
time_series <- environmental_stochasticity_ext(N0, tmax, possible_outcomes, probabilities) # runs the function
N0
plot(time_series, type="l", xlab="time", ylab="population size")
...
for (i in 1:sim) {# executes the function sim times
  results[, i] <- environmental_stochasticity_ext(N0, tmax, possible_outcomes, probabilities) # produces a time series and stores it in the i-th column of the matrix results
}
...
plot(results[, 1], type="l", ylim=c(0, max(time_series_arith, results)), col=cl[1], xlab="time", ylab="population size") # plots the result from the first simulation
for (i in 2:sim) {# adds the results from all other simulations to the same graph
...
```{r eval=FALSE}
plot(log(results[, 1]+1), type="l", ylim=c(0, log(max(time_series_arith, results)+1)), col=cl[1], xlab="time", ylab="ln(population size)") # plots the result from the first simulation on a log-scale
for (i in 2:sim) {# adds the results from all other simulations to the same graph
...
for (i in 1:sim) {
  results[, i] <- environmental_stochasticity_ext(N0, tmax, possible_outcomes, probabilities)
}
...
plot(log(results[, 1]+1), type="l", ylim=c(0, log(max(time_series_arith, results)+1)), col=cl[1], xlab="time", ylab="population size")
for (i in 2:sim) {
  lines(log(results[, i]+1), type="l", col=cl[i])
}
```
...
```{r eval=FALSE}
row_means <- rowMeans(log(results + 1))

# adds the mean of all time series to the previous graph
lines(row_means, type="l", col="Black")
```
...
$$
where $\bar \lambda = \sum_{i=0}^{t-1} \lambda(i)$ is the \textit{arithmetic mean} of the annual growth factors. In our current scenario, we have $\bar\lambda =$*lambda_mean*. Let us add this line.

```{r eval=FALSE}
# function computing a time series given that the population changes by the arithmetic mean lambda value
time_series_arith <- c() # creates an empty vector
time_series_arith[1] <- N0 # adds N0 as first entry
for (t in 1:tmax) {# for loop creating the time series
  time_series_arith[t+1] <- time_series_arith[t]*lambda_mean
}

# we can add mean of all time series to the previous graph
lines(log(time_series_arith+1), type="l", col="4")
```
You should now see a smooth blue line showing exponential growth. It shows how the population should grow if it would change by the factor $\bar \lambda$ from one year to the next. Well, this line does not seem to do a good job in predicting the average population trajectory (the black wiggly line).
...
lambda_geo <- (lambda_mean^2-deviation^2)^(1/2) # calculates the geometric mean lambda value according to the above formula
lambda_geo
time_series_geo <- c() # creates an empty vector
time_series_geo[1] <- N0 # adds N0 as first entry
for (t in 1:tmax) {# for loop creating the time series
  time_series_geo[t+1] <- time_series_geo[t]*lambda_geo
}

# we can add mean of all time series to the previous graph
lines(log(time_series_geo+1), type="l", col="Black")
```
